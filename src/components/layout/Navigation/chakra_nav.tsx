...
type DataProps = { data: GlobalHeader };

export default function ChakraNavBar({ data }: DataProps) {
  if (!data) return <></>;
  ...
  return (
    <div className="flex w-full justify-around items-center">
      <div className="flex items-center justify-between gap-6">
        <h4 className="select-none text-lg font-bold tracking-tight my-1 transition duration-150 ease-out transform">
          <Link
            href="/"
      ...
      <ul className="flex gap-2 sm:gap-4 lg:gap-8 tracking-[.002em] ">
        {data.nav &&
          data.nav.map((item, i) => {
            if (!item) return;
            const activeItem =
            ...
            return (
              ...
              >
                <Link
                  ...
                  href={`/${item.href}`}
                  className={`relative select-none	text-base inline-block tracking-wide transition duration-150 ease-out hover:opacity-100 py-1 px-2 ${
